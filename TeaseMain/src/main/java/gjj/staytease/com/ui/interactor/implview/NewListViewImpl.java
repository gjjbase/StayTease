...
import gjj.staytease.com.ui.view.news.NewDetailActivity;
...
public class NewListViewImpl extends ViewImpl implements NewListInteractor {
    ...
    public void setMsg(NewListFragment context, int position) {
        String content = contentlistEntityList.get(position).getDesc().trim();
        String title = contentlistEntityList.get(position).getTitle().trim();
        //.putExtra("desc", contentlistEntityList.get(position).getDesc()).putExtra("title", contentlistEntityList.get(position).getTitle()));
        context.startActivity(new Intent(context.getActivity(), NewDetailActivity.class).putExtra("desc", content).putExtra("title", title));
    }
